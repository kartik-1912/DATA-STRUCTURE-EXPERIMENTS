...
int product =1;
int prod_single(bst * root)
{
    if(root ==  NULL)
        return 0;
        //cout<<product<<endl;
    if((root->left==NULL && root->right!=NULL)||(root->left!=NULL && root->right==NULL))
        product=product*root->val;
    prod_single(root->left);
    prod_single(root->right);
...
int main()
{
  ...
  del(root,e);
  preo(root);
  int b=prod_single(root);
  cout<<"\nproduct of single child is "<<b;
  return 0;
}
